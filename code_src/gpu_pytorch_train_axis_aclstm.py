...
class acLSTM(nn.Module):
    ...
    #     return loss
    def calculate_loss(self, out_seq, groundtruth_seq):
        # Assuming out_seq and groundtruth_seq are sequences of angles in radians
        return angle_distance_loss(out_seq, groundtruth_seq)
...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False,
    ...
    predict_groundtruth_seq = torch.autograd.Variable(
        torch.FloatTensor(real_seq_dif_hip_x_z_np[:, 1:seq_len + 1].tolist())).cuda().view(real_seq_np.shape[0], -1)
...
read_weight_path = ""
write_weight_folder = "train_weight_aclstm_martial/axis_weights_ad/"
write_bvh_motion_folder = "train_tmp_bvh_aclstm_martial/axis_tmp_ad/"
dances_folder = "train_data_axis/martial/"
loss_log = "train_loss_aclstm_martial/axis_loss_ad.txt"
dance_frame_rate = 60
batch = 32
in_frame = 132
out_frame = 132
...
train(dances, dance_frame_rate, batch, 100, read_weight_path, write_weight_folder,
      write_bvh_motion_folder, in_frame, out_frame, hidden_size, total_iter=10001)