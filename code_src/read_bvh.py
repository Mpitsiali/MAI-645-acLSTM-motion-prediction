...
def parse_frames(bvh_filename):
   ...
   first_frame  = data_start + 3    # skips until motion the go throu lines
...
def get_euler_train_data(bvh_filename: str):

    euler_frames = parse_frames(bvh_filename)

    euler_frames_rads = euler_frames

    euler_frames_rads[:, 3:] = (np.sign(euler_frames_rads[:, 3:]) * (np.abs(euler_frames_rads[:, 3:]) % 360)) / 360

    return euler_frames_rads

def get_euler_data_from_npy(bvh_filename, train_data):

    format_filename = standard_bvh_file

    train_data[:, 3:] = (train_data[:, 3:]) * 360

    write_frames(format_filename, bvh_filename, train_data)


def get_axis_train_data(bvh_filename):
    data = parse_frames(bvh_filename)

    position = data[:, :3] * 0.01

    # Convert degrees to radians for the remaining data and reshape
    euler_angles = np.radians(data[:, 3:]).reshape(-1, 3)

    euler_matrix = rc.euler_angles_to_matrix(torch.Tensor(euler_angles), convention='XYZ')
    axis_ = rc.matrix_to_axis_angle(torch.Tensor(euler_matrix)).numpy()

    axis_ = axis_.reshape(len(data), -1)

    frame_data = np.concatenate((position, axis_), axis=1)
...
def get_axis_data_from_npy(bvh_filename, train_data):

    format_filename = standard_bvh_file

    position = train_data[:, :3] * 100
    quaternion_arrays = train_data[:, 3:].reshape(-1, 3)

    rotation_matrices = rc.axis_angle_to_matrix(torch.Tensor(quaternion_arrays))

    euler_angles = np.degrees(rc.matrix_to_euler_angles(rotation_matrices, convention='XYZ'))
    euler_angles = euler_angles.reshape(len(train_data), -1)

    frame_data = np.concatenate([position, euler_angles], axis=1)

    write_frames(format_filename, bvh_filename, frame_data)


def get_quad_train_data(bvh_filename):
    data = parse_frames(bvh_filename)
    ...
    euler_matrix = rc.euler_angles_to_matrix(torch.Tensor(euler_angles), convention='XYZ')
    quats_ = rc.matrix_to_quaternion(torch.Tensor(euler_matrix)).numpy()

    quats_ = quats_.reshape(len(data), -1)

    frame_data = np.concatenate((position, quats_), axis=1)

    return frame_data

def get_quad_data_from_npy(bvh_filename, train_data):

    format_filename = standard_bvh_file

    position = train_data[:, :3] * 100
    quaternion_arrays = train_data[:, 3:].reshape(-1, 4)

    rotation_matrices = rc.quaternion_to_matrix(torch.Tensor(quaternion_arrays))
...
def get_6d_train_data(bvh_filename):
    data = parse_frames(bvh_filename)

    position = data[:, :3] * 0.01

    euler_angles = np.radians(data[:, 3:]).reshape(-1, 3)
    matrix = rc.euler_angles_to_matrix(torch.Tensor(euler_angles), convention='XYZ')
    matrix_6d = rc.matrix_to_rotation_6d(torch.Tensor(matrix)).numpy()

    matrix_6d = matrix_6d.reshape(len(data), -1)

    frame_data = np.concatenate([position, matrix_6d], axis=1)

    return frame_data


def get_6d_data_from_npy(bvh_filename, train_data):
    format_filename = standard_bvh_file

    position = np.degrees(train_data[:, :3])
    joints = (train_data.shape[1] - 3) // 6

    data_6d = train_data[:, 3:].reshape(-1, joints, 6)
    matrix = rc.rotation_6d_to_matrix(torch.Tensor(data_6d.reshape(-1, 6)))

    euler_angles = rc.matrix_to_euler_angles(matrix, convention='XYZ').numpy()

    euler_angles = np.degrees(euler_angles.reshape(len(train_data), -1))
