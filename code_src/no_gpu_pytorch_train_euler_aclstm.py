...
class acLSTM(nn.Module):
    ...
    def init_hidden(self, batch):
        # c batch*(3*1024)
        c0 = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, self.hidden_size))))
        c1 = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, self.hidden_size))))
        c2 = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, self.hidden_size))))
        h0 = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, self.hidden_size))))
        h1 = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, self.hidden_size))))
        h2 = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, self.hidden_size))))
    ...
    def forward(self, real_seq, condition_num=5, groundtruth_num=5):
        ...
        out_seq = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, 1))))

        out_frame = torch.autograd.Variable(torch.FloatTensor(np.zeros((batch, self.out_frame_size))))
...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False,
    ...
    real_seq = torch.autograd.Variable(torch.FloatTensor(real_seq_dif_hip_x_z_np.tolist()))

    seq_len = real_seq.size()[1] - 1
    in_real_seq = real_seq[:, 0:seq_len]

    predict_groundtruth_seq = torch.autograd.Variable(
        torch.FloatTensor(real_seq_dif_hip_x_z_np[:, 1:seq_len + 1].tolist())).view(real_seq_np.shape[0], -1)
    ...
    if (save_bvh_motion == True):
        ...
        read_bvh.get_euler_data_from_npy(save_dance_folder + "%07d" % iteration + "_gt.bvh", gt_seq)
        read_bvh.get_euler_data_from_npy(save_dance_folder + "%07d" % iteration + "_out.bvh", out_seq)
...
# read_weight_path = ""
# write_weight_folder = "/content/MAI645_final_project/train_weight_aclstm_martial/"
# write_bvh_motion_folder = "/content/MAI645_final_project/train_tmp_bvh_aclstm_martial/"
# dances_folder = "/content/MAI645_final_project/train_data_xyz/martial/"
read_weight_path = ""
# write_weight_folder = "train_weight_aclstm_martial/euler_weights_angle_distance/"
# write_bvh_motion_folder = "train_tmp_bvh_aclstm_martial/euler_tmp_angle_distance/"
write_weight_folder = "train_weight_aclstm_martial/euler_weights/"
write_bvh_motion_folder = "train_tmp_bvh_aclstm_martial/euler_tmp/"
dances_folder = "train_data_euler/martial/"
loss_log = "train_loss_aclstm_martial/euler_loss.txt"
dance_frame_rate = 60
batch = 32
in_frame = 132
out_frame = 132
...
train(dances, dance_frame_rate, batch, 100, read_weight_path, write_weight_folder,
      write_bvh_motion_folder, in_frame, out_frame, hidden_size, total_iter=21000)