...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder):
    ...
    for b in range(batch):
        ...
        read_bvh.get_euler_data_from_npy(save_dance_folder + "out" + "%02d" % b + ".bvh", out_seq)
...
def test(dance_batch_np, frame_rate, batch, initial_seq_len, generate_frames_number, read_weight_path,
    ...
    dance_len_lst = get_dance_len_lst(dances)  # all dances
    ...
    # hold the dance and the start position of the sequaence
    gt_start_id = []
    gt_dances = []
    for b in range(batch):
        ...
        gt_dances.append(dances[dance_id])

        dance = dances[dance_id].copy()
        dance_len = dance.shape[0]

        start_id = random.randint(10, int(dance_len - initial_seq_len * speed - 10))
        if (start_id + generate_frames_number + initial_seq_len) > len(
                dance):  # make sure that it can generate generate_frames_number amount
            # change the start if the start id is at the last places
            start_id = start_id - generate_frames_number
        gt_start_id.append(start_id)
        sample_seq = []
    ...
    predicted_seq = generate_seq(dance_batch_np, generate_frames_number, model, write_bvh_motion_folder)

    calculated_loss = []
    for i, item in enumerate(gt_start_id):
        # comparing predicted seq to the target using MSE

        gt_dance_sample = gt_dances[i][item:+initial_seq_len + item + generate_frames_number - 1]
        read_bvh.get_euler_data_from_npy(write_bvh_motion_folder + "gt" + "%02d" % i + ".bvh", np.array(gt_dance_sample))
        predicted_dance_sample = predicted_seq[i]
        calculated_loss.append(mse(np.array(gt_dance_sample), predicted_dance_sample))

    for val in calculated_loss:
        print("MSE:", val[0])
        # print("Sum of Squared Errors:", val[1])
...
def mse(y_true, y_pred):
    ...
    sum_of_errors = np.sum(squared_errors)

    return mse_value, sum_of_errors



# Angle distance
# read_weight_path = "train_weight_aclstm_martial/euler_weights/0001000.weight"
# read_weight_path = "train_weight_aclstm_martial/euler_weights_ad/0020000.weight"
# write_bvh_motion_folder = "test_bvh_aclstm_martial/euler_ad/"
# dances_folder = "train_data_euler/martial/"

# MSE
read_weight_path = "train_weight_aclstm_martial/euler_weights_mse/0020000.weight"
write_bvh_motion_folder = "test_bvh_aclstm_martial/euler_mse/"
dances_folder = "train_data_euler/martial/"
