...
def generate_axis_traindata_from_bvh(src_bvh_folder, tar_traindata_folder):
    ...
    for bvh_dance_name in bvh_dances_names:
        ...
        if (name_len > 4):
            if (bvh_dance_name[name_len - 4: name_len] == ".bvh"):
                dance = read_bvh.get_axis_train_data(src_bvh_folder + bvh_dance_name)
                np.save(tar_traindata_folder + bvh_dance_name + ".npy", dance)



def generate_bvh_from_axis_traindata(src_train_folder, tar_bvh_folder):
    ...
    for dance_name in dances_names:
        ...
        if(name_len>4):
            if(dance_name[name_len-4: name_len]==".npy"):
                ...
                read_bvh.get_axis_data_from_npy(tar_bvh_folder + dance_name + ".bvh", np.array(dance2))
...
generate_axis_traindata_from_bvh("train_data_bvh/martial/","train_data_axis/martial/")

# Decode from positional to bvh
generate_bvh_from_axis_traindata("train_data_axis/martial/", "test_data_axis_bvh/martial/",)