...
# normalize and make to numpy array
def generate_euler_traindata_from_bvh(src_bvh_folder, tar_traindata_folder):
    ...
    for bvh_dance_name in bvh_dances_names:
        ...
        if(name_len>4):
            if(bvh_dance_name[name_len-4: name_len]==".bvh"): # checks if its .bvh file
                dance= read_bvh.get_euler_train_data(src_bvh_folder + bvh_dance_name)
    ...
    for dance_name in dances_names:
        ...
        if(name_len>4):
            if(dance_name[name_len-4: name_len]==".npy"):
                ...
                read_bvh.get_euler_data_from_npy(tar_bvh_folder+dance_name+".bvh", np.array(dance2))
