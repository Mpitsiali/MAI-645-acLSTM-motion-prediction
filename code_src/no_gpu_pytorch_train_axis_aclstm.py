...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False,
    ...
    if (print_loss == True):
        ...
        with open(loss_log, 'a') as file:
            file.write(str(loss.detach().cpu().numpy()) + ' ' + str(iteration) +'\n' )
    ...
    if (save_bvh_motion == True):
        ...
        read_bvh.get_axis_data_from_npy(save_dance_folder + "%07d" % iteration + "_gt.bvh", gt_seq)
        read_bvh.get_axis_data_from_npy(save_dance_folder + "%07d" % iteration + "_out.bvh", out_seq)
...
colab_path =  "/content/MAI645_final_project/"

# write_weight_folder = f"{colab_path}train_weight_aclstm_martial/"
# write_bvh_motion_folder = f"{colab_path}train_tmp_bvh_aclstm_martial/"
# dances_folder = f"{colab_path}train_data_xyz/martial/"
# read_weight_path = "train_weight_aclstm_martial/quad_weights/0001000.weight"
read_weight_path = ""
write_weight_folder = "train_weight_aclstm_martial/axis_weights_mse/"
write_bvh_motion_folder = "train_tmp_bvh_aclstm_martial/axis_tmp_mse/"
dances_folder = "train_data_axis/martial/"
loss_log = "train_loss_aclstm_martial/axis_loss_mse.txt"
dance_frame_rate = 60
batch = 32
in_frame = 132
out_frame = 132
